...
use warehouse "ELT_XS_WH";
use database "POUTINESHOP_RAW_DB";
use schema "RAW_DATA";

--------------------------------------------------------------------------------
-- products
--------------------------------------------------------------------------------
select * from "POUTINESHOP_RAW_DB"."RAW_DATA"."PRODUCTS"
where id in (11033, 425, 1009, 8903, 29000, 100, 18166, 683, 14717, 27398);

---- break not null test
update "POUTINESHOP_RAW_DB"."RAW_DATA"."PRODUCTS"
    set sku = null
...
-- order_items
--------------------------------------------------------------------------------
select * from "POUTINESHOP_RAW_DB"."RAW_DATA"."ORDER_ITEMS"
where id in (298433, 56567, 68410, 84202, 289395);

---- break accepted_values with test
update "POUTINESHOP_RAW_DB"."RAW_DATA"."ORDER_ITEMS"
    set status = 'Test'
...
-- inventory_items
--------------------------------------------------------------------------------
select * from "POUTINESHOP_RAW_DB"."RAW_DATA"."INVENTORY_ITEMS"
where id in (288038, 218960, 325238, 45766, 99912);

---- break not null test
update "POUTINESHOP_RAW_DB"."RAW_DATA"."INVENTORY_ITEMS"
    set product_id = NULL
    where id in (288038, 218960, 325238, 45766, 99912);
